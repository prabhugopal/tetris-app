...
(defn parse-the-game-state-to-rows
  "takes the game state as input, the game state should be representated in vector of hash-sets, each sets contains the occupied columns/cells of that partifular row. produces the number of rows or height of the resultant game state."
  [game-state]
  (count game-state))
...
(defn get-valid-row
  ...
  (let [new-row (pick-available-row shape col state)]
    (if (can-shape-fit? shape col new-row size)
      new-row
      (inc new-row))))
  ...
  (reduce
   (fn [state cells]
     (assoc state (first cells)
            (set (concat (nth state (first cells) nil)
                         (last cells)))))
   current-state rows-cols))
